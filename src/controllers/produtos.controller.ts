...
// Endpoint para criar produto durante solicitação (permite supervisor)
export const createProdutoDuringSolicitacao = async (req: RequestWithUser, res: Response) => {
  try {
    if (!req.user || !req.userRegra) {
      return res.status(401).json({ error: 'Usuário não autenticado' });
    }

    // Supervisor pode criar produtos durante solicitação
    // Diretor também pode usar este endpoint
    const produtoData = { ...req.body };
